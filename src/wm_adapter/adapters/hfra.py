...
class HFRASiteAdapter(nn.Module):
    """One split-branch Hybrid Fourier Residual Adapter insertion site."""
    ...
    def __init__(
        ...
        *,
        ...
        if fourier_enabled and rank < 2:
            raise ValueError(
                "Full HFRA requires rank >= 2 so the total bottleneck budget "
                "can be split between core and spectral branches"
            )
        ...
        self.core_rank = rank if not fourier_enabled else rank // 2
        self.spectral_rank = 0 if not fourier_enabled else rank - self.core_rank

        self.norm = nn.RMSNorm(embed_dim, elementwise_affine=False)
        self.core_down = nn.Linear(embed_dim, self.core_rank, bias=False)
        self.core_activation = nn.GELU()
        self.core_up = nn.Linear(self.core_rank, embed_dim, bias=False)
        nn.init.kaiming_uniform_(self.core_down.weight, a=math.sqrt(5))
        nn.init.zeros_(self.core_up.weight)

        if fourier_enabled:
            self.spectral_down = nn.Linear(
                embed_dim, self.spectral_rank, bias=False
            )
            self.spectral_activation = nn.GELU()
            self.spectral_up = nn.Linear(
                self.spectral_rank, embed_dim, bias=False
            )
            nn.init.kaiming_uniform_(
                self.spectral_down.weight, a=math.sqrt(5)
            )
            ...
            self.frequency_real = nn.Parameter(
                torch.ones(frequency_shape)
            )
            self.frequency_imag = nn.Parameter(
                torch.zeros(frequency_shape)
            )
            with torch.no_grad():
                self.frequency_real.add_(
                    torch.randn_like(self.frequency_real) * 1.0e-4
                )
            self.spectral_gate = nn.Parameter(torch.ones(()))
    ...
    def _spectral_residual(self, core: Tensor) -> Tensor:
        ...
        if rank != self.spectral_rank:
            raise RuntimeError(
                "HFRA spectral bottleneck changed width: "
                f"expected={self.spectral_rank}, actual={rank}"
            )
        ...
        frequency = torch.complex(
            self.frequency_real.float(),
            self.frequency_imag.float(),
        )
        filtered = coefficients * frequency.unsqueeze(0)
    ...
    def _deltas(self, patch_tokens: Tensor) -> tuple[Tensor, Tensor, Tensor]:
        normalized = self.norm(patch_tokens)
        core = self.core_activation(self.core_down(normalized))
        core_delta = self.core_up(core)
        if self.fourier_enabled:
            spectral_core = self.spectral_activation(
                self.spectral_down(normalized)
            )
            spectral = self._spectral_residual(spectral_core)
            spectral_delta = self.spectral_up(spectral)
            gated_spectral_delta = self.spectral_gate * spectral_delta
        else:
            spectral_delta = torch.zeros_like(core_delta)
            gated_spectral_delta = spectral_delta
        total_delta = core_delta + gated_spectral_delta
        return core_delta, spectral_delta, total_delta
    ...
    def forward(self, patch_tokens: Tensor) -> Tensor:
        ...
        core_delta, spectral_delta, total_delta = self._deltas(
            patch_tokens
        )
        denominator = (
            patch_tokens.float()
        ...
        self._latest_diagnostics = {
            "core_delta_ratio": (
                core_delta.float().square().mean().sqrt() / denominator
            ).detach(),
            "spectral_delta_ratio": (
                spectral_delta.float().square().mean().sqrt() / denominator
            ).detach(),
            "total_delta_ratio": (
                total_delta.float().square().mean().sqrt() / denominator
            ).detach(),
        }
        if self.fourier_enabled:
            ...
            self._latest_diagnostics.update(
                ...
                spectral_gate=self.spectral_gate.detach(),
            )
    ...
    def diagnostics(self, patch_tokens: Tensor) -> dict[str, Tensor]:
        ...
        values = {
            ...
            "spectral_delta_ratio": (
                spectral_delta.float().square().mean().sqrt()
                / denominator
            ),
            "total_delta_ratio": (
                total_delta.float().square().mean().sqrt() / denominator
...
class HybridFourierResidualAdapter(PEFTMethod):
    ...
    def __init__(
        ...
        *,
        ...
        if method_name not in {"hfra", "hfra_core_only"}:
            raise ValueError(
                f"Unsupported HFRA method name: {method_name!r}"
            )
    ...
    def config_dict(self) -> dict[str, Any]:
        return {
            "architecture": "split_core_spectral",
    ...
    def latest_diagnostics(self) -> dict[str, Tensor]:
        ...
        return {
            key: torch.stack(
                [item[key] for item in values if key in item]
            ).mean()
            for key in keys
        }
