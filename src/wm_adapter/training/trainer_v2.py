...
CHECKPOINT_SCHEMA_V2 = "wm_adapter_checkpoint_v2.2_clean_only"
...
class TrajectoryAdapterTrainer:
    def __init__(
        ...
        *,
        ...
        method: PEFTMethod,
        config: TrajectoryTrainingConfig,
        ...
        if config.loss_name != "unified_trajectory_mse":
            raise ValueError(
                f"V2 supports only unified_trajectory_mse, found {config.loss_name}"
            )
        if config.scheduler != "cosine":
            raise ValueError(
                f"V2 supports only cosine scheduling, found {config.scheduler}"
            )
        if config.views_per_window != 1:
            raise ValueError(
                "Clean-only training requires training.views_per_window=1"
            )
        ...
        if not parameters:
            raise RuntimeError(
                f"Method {method.method_name} has no trainable parameters"
            )
        ...
        if leaked:
            raise RuntimeError(
                f"Frozen base contains trainable parameters: {leaked}"
            )
        ...
        for name, parameter in method.named_parameters():
            if (
                ...
                or id(parameter) not in trainable_ids
            ):
                continue
            zero_decay = any(
                marker in name
        ...
        def schedule(step: int) -> float:
            ...
            progress = (
                ...
                config.max_optimizer_steps - config.warmup_steps,
                1,
            )
            cosine = 0.5 * (
        ...
        if config.precision == "bf16":
            if (
                ...
                raise RuntimeError(
                    "V2 bf16 training requires CUDA bf16 support"
                )
            ...
            raise ValueError(
                f"Unsupported V2 precision {config.precision!r}"
            )
    ...
    def trajectory_loss(
        ...
        predicted = torch.cat(
            (adapted_context, predicted_future),
            dim=1,
        )
        if predicted.shape != clean_target.shape:
            raise RuntimeError(
                "Unified clean trajectory shape mismatch: "
                f"predicted={tuple(predicted.shape)}, "
                f"target={tuple(clean_target.shape)}"
            )

        loss = F.mse_loss(predicted, clean_target)
        method_diagnostics = (
        ...
        diagnostics = {
            "clean_mse": loss.detach(),
            "context_mse": F.mse_loss(
                predicted[:, :3].float(),
                clean_target[:, :3].float(),
            ).detach(),
    ...
    def _identity_invariant(
        ...
        with torch.no_grad(), self._autocast():
            base = self.backend.encode_from_site(
                tokens,
                self.backend.num_encoder_blocks // 2,
                BaseMethod().to(self.device),
            )
            adapted = self.backend.encode_from_site(
                tokens,
                self.backend.num_encoder_blocks // 2,
                self.method,
            )
        error = float(
            (base.float() - adapted.float()).abs().max().cpu()
        )
        atol, rtol = (
            (1.0e-6, 1.0e-5)
        ...
        if not torch.allclose(
            ...
            raise RuntimeError(
                "V2 identity invariant failed for "
                f"{self.method.method_name}: "
                f"max_abs_error={error}, atol={atol}, rtol={rtol}"
            )
        print(
            "Identity invariant passed: "
            f"method={self.method.method_name}, max_abs_error={error}",
            flush=True,
        )
        return error
        ...
        *,
        ...
        while optimizer_step < self.config.max_optimizer_steps:
            ...
            with self._autocast():
                ...
                backward_loss = (
                    loss / self.config.gradient_accumulation
                )
            ...
            grad_norm = torch.nn.utils.clip_grad_norm_(
                list(self.method.trainable_parameters()),
            ...
            if (
                optimizer_step % 10 == 0
                or optimizer_step in {1, self.config.max_optimizer_steps}
            ):
                processed_windows = (
                    micro_step * self.config.microbatch_windows
                )
                print(
                    "TRAIN_PROGRESS "
                    f"step={optimizer_step} "
                    f"total={self.config.max_optimizer_steps} "
                    f"loss={final['loss']:.8f} "
                    f"clean_mse={final['clean_mse']:.8f} "
                    f"context_mse={final['context_mse']:.8f} "
                    f"future_mse={final['future_mse']:.8f} "
                    f"lr={self.optimizer.param_groups[0]['lr']:.9f} "
                    f"grad_norm={final['grad_norm']:.6f} "
                    f"samples_per_sec={processed_windows / elapsed:.3f} "
                    f"core_delta_ratio={final['core_delta_ratio']:.8f} "
        ...
        payload = {
            ...
            "encoder_checkpoint_sha256": self.backend.encoder_checkpoint_sha256,
            "encoder_name": self.backend.encoder_name,
            "predictor_depth": self.backend.predictor_depth,
            ...
            "scheduler_config": {
                ...
                "minimum_lr": self.config.minimum_lr,
            },
