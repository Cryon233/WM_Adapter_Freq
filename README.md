...
The official RoboCasa365 LeRobot v3 reader uses metadata-sorted episode IDs, the declared agent-view video stream, and the last seven components of the 12-D RoboCasa action. Those normalized manipulator actions pass through the same recorded RoboCasa controller transform before entering the JEPA-WM cache. Strict closed-loop preflight separately requires an external `model_xml` and full MuJoCo simulator state. A policy-only LeRobot release containing only the 16-D `observation.state` remains useful as an inspected offline source but is rejected for formal planning with a precise error; that vector is never mislabeled as simulator state. Dataset-directory fingerprints hash the deterministic set of non-cache metadata, Parquet, and video files.

The shell wrapper sources `${CONDA_SH:-$HOME/anaconda3/etc/profile.d/conda.sh}` and activates `${CONDA_ENV:-wm-a100}` without calling `conda info`; override either variable for a non-default server installation. A missing `env_jepa.sh` is a hard error, while `env_libero.sh` remains optional at shell startup so `--dry-run` can report missing LIBERO resources explicitly.
