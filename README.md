...
The shell wrapper sources `${CONDA_SH:-$HOME/anaconda3/etc/profile.d/conda.sh}` without calling `conda info`. It reuses an already active non-base Conda environment, otherwise activates `wm-a100`; set `WM_ADAPTER_CONDA_ENV` for an explicit override. The generic `CONDA_ENV` variable is intentionally ignored because unrelated shell setup commonly exports it. A missing `env_jepa.sh` is a hard error, while `env_libero.sh` remains optional at shell startup so `--dry-run` can report missing LIBERO resources explicitly.
